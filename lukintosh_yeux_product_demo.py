...
# ============================================================
# LUKINTOSH YEUX — FACE + PUPIL PRODUCT DEMO
# ============================================================
#
# Mantém as alterações principais:
# - rosto/cabeça = movimento base
# - pupila/íris = mira fina
# - blink = clique esquerdo
# - dwell desligado por padrão
# - sem clique direito
# - Windows SendInput/SetCursorPos como backend atual
# - arquitetura pronta para trocar depois por Virtual HID Driver
# - preparada para futuramente virar multiplataforma
#
# Futuro:
# - Windows: Lukintosh Yeux Virtual HID Mouse Driver
# - macOS: Quartz Event / DriverKit HID
# - Linux: uinput / evdev
# - ChromeOS: WebHID / extensão / Lacros dependendo do ambiente
# - Consoles: SDK oficial de cada plataforma
#
# Comandos:
# Q / ESC -> sair
# P       -> pausar
...
# D       -> dwell click on/off, vem desligado por padrão
# F       -> fast/balanced
...
#
# Arquivo obrigatório na mesma pasta:
...
class MouseBackend:
    ...
    Hoje:
    - WindowsNativeMouseBackend usa SetCursorPos + SendInput.
    - FallbackMouseBackend usa pyautogui.

    Futuro:
    - WindowsHIDMouseBackend: driver virtual HID da Lukintosh Yeux.
    - MacOSMouseBackend: Quartz / DriverKit.
    - LinuxMouseBackend: uinput / evdev.
    - ChromeOSMouseBackend: WebHID/extensão, quando viável.
    """
...
class WindowsNativeMouseBackend(MouseBackend):
    """
    Backend atual de produto/demo para Windows.

    Move:
    - SetCursorPos

    Clique:
    - SendInput com LEFTDOWN/LEFTUP

    Importante:
    - Não há botão direito.
    - Blink sempre chama left_click().
    - Para aparecer no Gerenciador de Dispositivos como mouse real,
      depois este backend deve ser trocado por WindowsHIDMouseBackend.
    """
...
class FallbackMouseBackend(MouseBackend):
    """
    Backend temporário para plataformas não-Windows.
    Não é backend final de produto.
    """
...
class WindowsHIDMouseBackendPlaceholder(MouseBackend):
    """
    Placeholder do futuro driver HID virtual.

    Arquitetura futura:
    Yeux App
      -> Yeux Input Service
      -> Yeux Virtual HID Mouse Driver
      -> Windows HID stack
      -> Gerenciador de Dispositivos: "Lukintosh Yeux Virtual Mouse"

    Este placeholder existe para o código já nascer preparado.
    """

    def move_to(self, x: int, y: int) -> None:
        raise NotImplementedError("Virtual HID driver ainda não implementado.")
...
def create_mouse_backend() -> MouseBackend:
    if platform.system().lower() == "windows":
        return WindowsNativeMouseBackend()

    return FallbackMouseBackend()
...
class CalibrationConfig:
    ...
    file_path: str = "lukintosh_yeux_calibration.json"
...
class DwellClickConfig:
    # Desligado por padrão porque você pediu blink = click.
    enabled: bool = False
...
@dataclass
class OverlayConfig:
    window_name: str = "Lukintosh Yeux Product Demo"
...
class DemoConfig:
    ...
    fast_mode: bool = True
    log_file: str = "lukintosh_yeux_metrics.jsonl"
    ...
    def run(self) -> None:
        ...
        while self.state.snapshot()["running"]:
            ...
            try:
                ...
                timestamp_ms = int(packet.capture_ms)
                ...
                if result.face_landmarks:
                    ...
                    if done:
                        status = "tracking"
                        msg = "Lukintosh Yeux tracking active" if has_iris else "Head tracking fallback"
                    else:
                        status = "calibrating"
...
class CursorWorker:
    def __init__(
        ...
        self.last_sent_px = -9999
        self.last_sent_py = -9999
    ...
    def update_target(self, pose: PosePacket, fast_mode: bool) -> None:
        ...
        if fast_mode:
            ...
            slow_alpha = self.config.slow_alpha * 0.72
            fast_alpha = self.config.fast_alpha * 0.68
    ...
    def norm_to_screen(
        ...
        taskbar_snap = self.config.taskbar_snap_zone

        left_safe_x = self.config.left_safe_x
        right_safe_x = self.screen_w - self.config.right_safe_padding
        top_safe_y = self.config.top_safe_y

        taskbar_hover_y = int(clamp(taskbar_hover_y, 18, 100))
        bottom_taskbar_y = self.screen_h - taskbar_hover_y
    ...
    def apply_taskbar_boost(self) -> None:
        if self.target_y >= 1.0 - self.config.taskbar_boost_zone:
            strength = self.target_y - (1.0 - self.config.taskbar_boost_zone)
            strength = clamp(strength / self.config.taskbar_boost_zone, 0.0, 1.0)
            self.vy += 0.0025 + 0.0038 * strength
    ...
    def apply_hover_freeze(
        ...
        if self.hover_freeze_started_ms <= 0:
            self.hover_freeze_started_ms = now
            self.hover_freeze_px = px
            self.hover_freeze_py = py
            self.hover_frozen = False
            return px, py, False

        movement = math.hypot(px - self.hover_freeze_px, py - self.hover_freeze_py)

        if movement > self.config.taskbar_lock_move_reset_px:
            self.hover_freeze_started_ms = now
            self.hover_freeze_px = px
            self.hover_freeze_py = py
            self.hover_frozen = False
        ...
        elapsed = now - self.hover_freeze_started_ms

        if elapsed >= self.config.taskbar_lock_min_ms:
            self.hover_frozen = True
            return self.hover_freeze_px, self.hover_freeze_py, True
    ...
    def send_mouse_move(self, px: int, py: int, frozen: bool) -> None:
        ...
        if abs(px - self.last_sent_px) < eps and abs(py - self.last_sent_py) < eps:
            return

        self.mouse.move_to(px, py)

        self.last_sent_px = px
        self.last_sent_py = py
    ...
    def move_cursor(
        ...
        self.apply_taskbar_stabilization(taskbar_region)
...
class OverlayWorker:
    ...
    def draw_header(self, frame: np.ndarray, snap: Dict[str, Any]) -> None:
        draw_panel(frame, 12, 12, 470, 210, alpha=0.56)
        ...
        draw_badge(frame, badge_text, 28, 118, badge_color)

        mode_text = "FAST" if snap["fast_mode"] else "BALANCED"
        draw_badge(frame, mode_text, 158, 118, (255, 255, 255))

        iris_text = "PUPIL ON" if snap["has_iris"] else "HEAD ONLY"
        iris_color = (0, 255, 255) if snap["has_iris"] else (120, 120, 120)
        draw_badge(frame, iris_text, 238, 118, iris_color)

        click_text = "BLINK=LEFT CLICK" if snap["blink_click_enabled"] else "BLINK OFF"
        draw_badge(frame, click_text, 28, 152, (255, 230, 160))
        ...
        draw_badge(frame, dwell_text, 205, 152, dwell_color)
    ...
    def draw_help(self, frame: np.ndarray, snap: Dict[str, Any]) -> None:
        ...
        controls = [
            ...
            "C blink left-click   D dwell click   F fast/balanced   B edge assist",
    ...
    def draw_calibration(self, frame: np.ndarray, snap: Dict[str, Any]) -> None:
        ...
        put_text(
            frame,
            "Calibrating Lukintosh Yeux",
            (int(w * 0.5) - 215, int(h * 0.5) - 45),
            scale=0.65,
            color=(255, 255, 255),
            thickness=2,
        )
        ...
        put_text(
            frame,
            "Move head + look with pupils to all corners",
            (int(w * 0.5) - 210, int(h * 0.5) - 6),
            scale=0.48,
            color=(0, 255, 255),
            thickness=1,
        )
    ...
    def handle_key(self, key: int) -> None:
        ...
        if key in (ord("r"), ord("R")):
            ...
            self.state.set(
                ...
                user_message="Recalibrating Lukintosh Yeux...",
            )
    ...
    def try_load_calibration_on_start(self) -> None:
        ...
        if ok:
            self.state.set(
                ...
                user_message="Lukintosh Yeux calibration loaded",
            )
        else:
            self.state.set(
                calibrated=False,
                calibrating=True,
                calibration_progress=0.0,
                user_message="Starting Lukintosh Yeux calibration",
            )
